# dealer deals cards to player
# cards display values
# player either hits, stays, or splits
# cards totaled up
# dealers turn
# if dealer busts, or has less value than human- human wins
# display winner
require 'rubygems'
require 'pry'
...
class Blackjack
  ...
  def initialize
    @deck = Deck.new
    @player = Player.new("Player's Name")
    @dealer = Dealer.new
  end
  ...
  def blackjack_or_bust?(player_or_dealer)
    if player_or_dealer.total == BLACKJACK_AMOUNT
      if player_or_dealer.is_a?(Dealer)
        puts "Sorry, dealer hit blackjack. Your done for"
      else
        puts "Congrats, you hit a blackjack"
      end
      play_again?
  ...
  def dealer_turn
    ...
    def who_won?
      if player.total > dealer.total
        ...
        puts "sorry the computer beat you sucker"
      else 
        puts "its a tie game!"
      end
      play_again?
